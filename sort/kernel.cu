...
int main(void){
    ...
    // host pointers
    int *array, *h_gpu_result, *array_q, *array_b;

    // malloc an array
    array = (int*)malloc(size*sizeof(int));
    h_gpu_result = (int*)malloc(size*sizeof(int));
    ...
    //cuda malloc an array
    cudaError error;
    error = cudaMalloc((void**)&d_array, size*sizeof(int));
    ...
    while(sem){
        ...
        cudaMemcpy(h_gpu_result, d_array, size*sizeof(int), cudaMemcpyDeviceToHost);
        // print_array(h_gpu_result, size);
        correct = true;
        
        for(int i=0;i<size-1;i++){
            if(h_gpu_result[i] > h_gpu_result[i+1]){
                correct = false;
                break;
            }
        }
    ...
    checker = check_result(h_gpu_result, size);
    if(checker == true){
