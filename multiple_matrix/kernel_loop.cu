...
Each thread corresponds to a given element of the resulting matrix.
What follows is an iteration over the rows of matrix A and the columns 
of matrix B where the element of matrix A is multiplied with the element 
of matrix B and the result is summed to the resulting matrix.
*/
...
int main(void){
    ...
    kernel<<<grid, block>>>(d_arr_a, d_arr_b, d_arr_c, m, n, k);
