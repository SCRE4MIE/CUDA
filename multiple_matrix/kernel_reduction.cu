...
__global__ void kernel(int*arr_a, int*arr_b, int*arr_c, int m_i, int n_i, int k_i, long long int num_of_operations, int offset_i){

    long long int offset = (long long int)offset_i;
    ...
    long long int start_loop = threadId * offset;
    long long int stop_loop = start_loop + (long long int)offset;
    if (threadId < num_of_operations){
        long long int m = (long long int)m_i;
        long long int n = (long long int)n_i;
        for(long long int i=start_loop;i<stop_loop;i++){
            if(i < num_of_operations * offset){
                int x_a = i / (m*n);
                int y_a = i % m;
                ...
                atomicAdd(&(arr_c[x_c * n + y_c]), multiple);
            }
        }
...
int main(void){
    ...
    // kernel launch
    long long int num_of_operations = (long long int)(((long long int)n * (long long int)n * (long long int)m) / (long long int)offset);

    dim3 block(32,32,1);
    dim3 grid(243,243,1);

    kernel<<<grid, block>>>(d_arr_a, d_arr_b, d_arr_c, m, n, k, num_of_operations, offset);
